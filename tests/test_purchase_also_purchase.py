...
class PurchaseAlsoPurchaseTestCase(RecsTestCaseWithData):
    ...
    def test_7_day_purchase_also_purchase_account_level_offline_pos(self):
        recsets = recs_models.RecommendationSet.objects.filter(
            Q(algorithm='purchase_also_purchase',
              account=self.account,
              lookback_days=7,
...
class PurchaseAlsoPurchaseFiltersTestCase(RecsTestCaseWithData):
    @classmethod
    @patch_invalidations
    def setUpClass(cls):
        super(PurchaseAlsoPurchaseFiltersTestCase, cls).setUpClass()
        # initializing purchase_also_purchase recsets
        # filters with 30 day lookback
    ...
    def test_30_day_purchase_also_purchase_account_level_with_default_attribute_added_to_collab_sku_ranks_query(self):
        recsets = recs_models.RecommendationSet.objects.filter(
            Q(algorithm='purchase_also_purchase',
              account=self.account,
              lookback_days=30,
    ...
    def test_7_day_purchase_also_purchase_account_level_with_multiple_filters_on_same_attribute(self):
        recsets = recs_models.RecommendationSet.objects.filter(
            Q(algorithm='purchase_also_purchase',
              account=self.account,
              lookback_days=7,
        ...
        self._run_collab_recs_test('purchase_also_purchase', 7, recsets, expected_results, account=self.account)
