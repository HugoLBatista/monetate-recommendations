...
class SubsequentlyPurchasedTestCase(RecsTestCaseWithData):
    @classmethod
    @patch_invalidations
    def setUpClass(cls):
        super(SubsequentlyPurchasedTestCase, cls).setUpClass()

        # initializing subsequently_purchased recsets
        ...
        with invalidation_context():
            for recset in recsets_to_create:
                rec = recs_models.RecommendationSet.objects.create(
                    algorithm='subsequently_purchased',
    ...
    def test_30_day_subsequently_purchased_account_level(self):
        recsets = recs_models.RecommendationSet.objects.filter(
            Q(algorithm='subsequently_purchased',
        ...
        recs1_expected_result = [
            ('TP-00001', [('SKU-00003', 1), ('SKU-00002', 2)]),
            ('TP-00004', [('SKU-00002', 1), ('SKU-00003', 2), ('SKU-00001', 3)]),
        ...
        self._run_collab_recs_test('subsequently_purchased', 30, recsets,
                                   expected_results, account=self.account, purchase_data_source="online")

    def test_30_day_subsequently_purchased_account_level_market(self):
        recsets = recs_models.RecommendationSet.objects.filter(
            Q(algorithm='subsequently_purchased',
              account=self.account,
              lookback_days=30,
    ...
    def test_30_day_subsequently_purchased_account_level_retailer_market(self):
        recsets = recs_models.RecommendationSet.objects.filter(
            Q(algorithm='subsequently_purchased',
              account=self.account,
              lookback_days=30,
    ...
    def test_7_day_subsequently_purchased_account_level(self):
        recsets = recs_models.RecommendationSet.objects.filter(
            Q(algorithm='subsequently_purchased',
              account=self.account,
              lookback_days=7,
        ...
        for r in recsets:
            expected_results[r.id] = [
                ('TP-00004', [('SKU-00002', 1), ('SKU-00001', 2)]),
                ('TP-00005', [('SKU-00004', 1), ('SKU-00002', 2), ('SKU-00001', 3)]),
                ('TP-00002', [('SKU-00001', 1)]),
            ]
        self._run_collab_recs_test('subsequently_purchased', 7, recsets, expected_results,
                                   account=self.account, purchase_data_source="online")

    def test_7_day_subsequently_purchased_account_level_offline_pos(self):
        recsets = recs_models.RecommendationSet.objects.filter(
            Q(algorithm='subsequently_purchased',
              account=self.account,
              lookback_days=7,
    ...
    def test_2_day_subsequently_purchased_account_level_retailer_market_offline_pos(self):
        recsets = recs_models.RecommendationSet.objects.filter(
            Q(algorithm='subsequently_purchased',
              account=self.account,
              lookback_days=2,
        ...
        recs10_expected_result =[
            ...
            ('TP-00002', [('SKU-00004', 1), ('SKU-00001', 2)]),
        ]
        expected_results_arr = [recs10_expected_result]
        expected_results = {}
        for index, r in enumerate(recsets):
            expected_results[r.id] = expected_results_arr[index]
        self._run_collab_recs_test('subsequently_purchased', 2, recsets,
                                   expected_results, account=None, market=None, retailer=self.retailer_id,
                                   purchase_data_source="offline")


    def test_30_day_subsequently_purchased_account_level_online_offline_pos(self):
        recsets = recs_models.RecommendationSet.objects.filter(
            Q(algorithm='subsequently_purchased',
              account=self.account,
              lookback_days=30,
        ...
        self._run_collab_recs_test('subsequently_purchased', 30, recsets, expected_results,
                                   account=self.account, purchase_data_source="online_offline")

    def test_2_day_subsequently_purchased_account_level_market_online_offline_pos(self):
        recsets = recs_models.RecommendationSet.objects.filter(
            Q(algorithm='subsequently_purchased',
              account=self.account,
              lookback_days=2,
